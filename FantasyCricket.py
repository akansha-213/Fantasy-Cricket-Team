...
class Ui_MainWindow(object):
    ...
    # Sql command for Connecting Fantasy_Game Database
    def connectdb(self):
        conn=sqlite3.connect('Fantasy_Game.db')
        cur=conn.cursor()
        return cur

    # Method for displaying Batsmen in ListWidget(playersList)
    def BAT_players(self):
        cur=self.connectdb()
        self.PlayersList.clear()
        cur.execute('Select Players from Stats where Category="Bat" ')
    ...
    def BOW_players(self):
        cur=self.connectdb()
        self.PlayersList.clear()
        cur.execute('Select Players from Stats where Category="Bowl" ')
    ...
    def AR_players(self):
        cur=self.connectdb()
        self.PlayersList.clear()
        cur.execute('Select Players from Stats where Category="AR" ')
        for i in objcricket.fetchall():
            if i[0] not in self.playersSelectedList:
                self.PlayersList.addItem(i[0])
    ...
    def WK_players(self):
        cur=self.connectdb()
        self.PlayersList.clear()
        cur.execute('Select Players from Stats where Category="WK" ')
        for i in objcricket.fetchall():
            if i[0] not in self.playersSelectedList:
                self.PlayersList.addItem(i[0])
    ...
    def open_team(self):
        ...
        if ok == True:
            ...
            cur=self.connectdb()
            cur.execute('Select Players,Name,value from Teams')
            for i in cur.fetchall():
                if i[1] == self.team:
                    self.Pointused+=i[2]
            ...
            cur.execute('Select Players,Category from Stats')
            for i in objcricket.fetchall():
    ...
    def save_team(self):
        ...
        if self.entername.text() !="":
            if self.SelectedPlayers.count() == 11:
                ...
                cur.execute('Select Players,Category,Value from Stats')
                self.k=0
                if self.entername.text() not in self.teamlist:
                    for i in cur.fetchall():
                        if i[0] in self.playersSelectedList:
                            cur.execute('''INSERT INTO Teams(Players,Name,value)
    ...
    def delete_team(self):
        ...
        if ok == True:
            ...
            if self.team in self.AllTeams:
                cur.execute('Delete from Teams where Name="%s"'%(self.team))
                conn.commit()
...
6. \bYou can Directly create New Team after saving (No need to click 'New Team' Again)\b'''
    ...
    def about(self):
        ...
        mess.setWindowTitle('Welcome to Fantasy Cricket Game')
        mess.setStyleSheet("background-color: #E0FFFF")
        message='''Hi This is Fantasy Cricket Game\n \aCreated By \bRohan Singh\b
\aIn this Game, You can Create, Open, and Delete Teams'''
        mess.setText(message)
        mess.setStandardButtons(QtWidgets.QMessageBox.Ok)
        font = QtGui.QFont()
        font.setFamily("Comic Sans MS")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.BAT_label_2.setText(_translate("MainWindow", "Batsman"))
        self.BOW_label_2.setText(_translate("MainWindow", "Bowlers"))
        self.label_6.setText(_translate("MainWindow", "Your Selection"))
        self.teamname_label.setText(_translate("MainWindow", "Team Name :"))
        self.WK_label_2.setText(_translate("MainWindow", "Wicket-keeper"))
        ...
        self.AR_label_2.setText(_translate("MainWindow", "AllRounders"))
