...
"font: 75 12pt \"MS Shell Dlg 2\";")
    ...
    def menufunction(self,action):
        ...
        if txt=="Open Team":
            ...
            self.openteam()
        if txt=="Team Score":
            from Score_points import Ui_Dialog
    ...
    def saveteam(self,nm,string,val):
        """
        nm:Name of team
        string:List of players in team seperated by commas
    ...
    def openteam(self):
        ...
        for i in range(count):
            ...
            sql="select Category from Stats where Players='"+player+"'"
            cur=conn.execute(sql)
            row=cur.fetchone()
            ctgr=row[0]
    ...
    def fillList(self,ctgr):
        ...
        if self.l1.text()=='???':
            ...
            return
        self.list1.clear()
        cursor = conn.execute("SELECT Players from Stats where Category='"+ctgr+"'")
        for row in cursor:
            selected=[]
    ...
    def removelist2(self, item):
        ...
        cursor = conn.execute("SELECT Players,Value,Category from Stats where Players='"+item.text()+"'")
        row=cursor.fetchone()
        self.avl=self.avl+int(row[1])
        self.used=self.used-int(row[1])
        ctgr=row[2]
...
3. Select 'Open Team' in 'Manage Team' or Click ctrl+O to Open Team\n'''
        ...
        mess.setStandardButtons(QtWidgets.QMessageBox.StandardButton.Ok)
    ...
    def rules(self):
        mess=QtWidgets.QMessageBox()
        mess.setWindowTitle('Welcome To Cricket Fantasy Game')
        message='''\t\bHere are some Rules:\b\n
